...
public sealed record AgentProfile(bool Interactive, int MaxOutputTokens)
{
    ...
    public static AgentProfile Chat(int maxOutputTokens = 4096) => new(Interactive: true, MaxOutputTokens: maxOutputTokens);

    /// <summary>Autonomous workflow run: no user prompts/charts, send_report expected.</summary>
    public static AgentProfile Workflow(int maxOutputTokens = 4096) => new(Interactive: false, MaxOutputTokens: maxOutputTokens);
}
